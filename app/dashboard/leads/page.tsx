...
import { Avatar, Tag } from "@/components/man/primitives";
import { Button } from "@/components/ui/button";
import { Search, Phone, Calendar, Send, Sparkles } from "lucide-react";
  ...
  return (
    <OwnerShell active="leads">
      <div className="flex h-[calc(100vh-64px)]">
        {/* Conversation list */}
        <div className="flex w-[320px] flex-shrink-0 flex-col" style={{ borderRight: "1px solid var(--card-edge)", background: "var(--card)" }}>
          <div className="p-4" style={{ borderBottom: "1px solid var(--card-edge)" }}>
            <div className="t-h4" style={{ color: "var(--ink-900)" }}>Lead inbox</div>
            <div className="mt-3 flex items-center gap-2 rounded-[10px] border px-3 py-2" style={{ borderColor: "var(--card-edge)" }}>
          ...
          <div className="flex-1 overflow-auto">
            {LEADS.map((l) => {
              const on = l.id === sel;
              return (
                <button key={l.id} onClick={() => setSel(l.id)} className="flex w-full items-start gap-3 px-4 py-3.5 text-left" style={{ borderBottom: "1px solid var(--card-edge)", background: on ? "var(--moss-50)" : "transparent" }}>
        ...
        <div className="flex flex-1 flex-col" style={{ background: "var(--paper)" }}>
          <div className="flex items-center gap-3 px-6 py-3.5" style={{ borderBottom: "1px solid var(--card-edge)", background: "var(--card)" }}>
            ...
            <Button variant="outline" size="sm"><Calendar className="mr-1.5 h-4 w-4" /> Book</Button>
          </div>

          <div className="flex-1 space-y-3 overflow-auto px-6 py-5">
            {active.thread.map((m, i) => (
              <div key={i} className={`flex ${m.me ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[70%] rounded-[14px] px-3.5 py-2.5" style={m.me ? { background: "var(--moss-700)", color: "var(--bone)" } : { background: "var(--card)", border: "1px solid var(--card-edge)", color: "var(--ink-900)" }}>
          ...
          {/* Composer */}
          <div className="px-6 py-4" style={{ borderTop: "1px solid var(--card-edge)", background: "var(--card)" }}>
