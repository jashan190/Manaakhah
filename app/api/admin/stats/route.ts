...
export async function GET(req: Request) {
  try {
    ...
    const stats = {
      ...
      businessesByStatus: {
        PUBLISHED: businesses.filter((b: any) => b.status === "PUBLISHED").length,
        DRAFT: businesses.filter((b: any) => b.status === "DRAFT").length,
        PENDING_APPROVAL: businesses.filter((b: any) => b.status === "PENDING_APPROVAL").length,
      ...
      reviewsByStatus: {
        ...
        REMOVED: reviews.filter((r: any) => r.status === "REMOVED").length,
      },
      bookingsByStatus: {
